...
def home(request):
    ...
    # Get groups that the user is not a member of and that the user has not already requested to join
    available_groups = Group.objects.exclude(members=user).exclude(join_requests__user=user)
    context = {
